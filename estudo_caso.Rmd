...
Os gráficos diagnósticos deste primeiro modelo podem ser vistos na figura \ref{fig:fit}. Como se pode ver nesta figura, os pontos 7 e 19 encontram-se bem afastados da média e foram excluídos do modelo final.
...
Um lote com as mesmas características do lote paradigma, porém com 5m a mais de frente, segundo o modelo, vale R\$`r brformat(p1)`/$m^2$ ($\approx$ `r brformat(coef(fit1)[1])` + `r brformat(coef(fit1)[2], digits = 3)`.5m). No modelo original, vale `r brformat(p1_orig)`/$m^2$.

Um lote com as mesmas características do lote paradigma, porém com 45m de profundidade, segundo o modelo, vale R\$`r brformat(p2)`/$m^2$ ($\approx$ `r brformat(coef(fit1)[1])` `r brformat(coef(fit1)[3], digits = 3)`.15m). No modelo original, vale `r brformat(p2_orig)`/$m^2$.
...
Um lote com as mesmas características do lote paradigma, porém com declive de 10%, segundo o modelo, vale R\$`r brformat(p4)`/$m^2$ . No modelo original, vale `r brformat(p4_orig)`/$m^2$ ($\approx$ `r brformat(coef(fit1)[1])` `r brformat(coef(fit)[4], digits = 3)`.$(-0,10)^2$ `r brformat(coef(fit)[5], digits = 3)`.$(-0,10)^3$ = `r brformat(coef(fit1)[1])` `r brformat((-.1)^2*coef(fit)[4], digits = 3)` + `r brformat((-.1)^3*coef(fit)[5], digits = 3)`).

Um lote com as mesmas características do lote paradigma, porém com aclive de 10%, segundo o modelo, vale R\$`r brformat(p5)`/$m^2$ . No modelo original, vale `r brformat(p5_orig)`/$m^2$ ($\approx$ `r brformat(coef(fit1)[1])` `r brformat(coef(fit)[4], digits = 3)`.$0,10^2$ `r brformat(coef(fit)[5], digits = 3)`.$0,10^3$ = `r brformat(coef(fit1)[1])` `r brformat((.1)^2*coef(fit)[4], digits = 3)` `r brformat((.1)^3*coef(fit)[5], digits = 3)`).

Finalmente, um lote com as mesmas características do lote paradigma, porém em terreno pantanoso, segundo o modelo, vale R\$`r brformat(p6)`/$m^2$ ($\approx$ `r brformat(coef(fit1)[1])` `r brformat(coef(fit1)[6])`). No modelo original, vale `r brformat(p6_orig)`/$m^2$.
...
|:--------------|--------------------------:|----------------------------:|-----------------------------:|-----------------------------:|----------:|----:|
| Paradigma     |`r brformat(coef(fit1)[1])`| 0                           | 0                            | 0                            |0          |54,27|
| (1)+5m frente |`r brformat(coef(fit1)[1])`|`r brformat(5*coef(fit1)[2])`| 0                            | 0                            |0          |59,25|
| (2)+15m fundo |`r brformat(coef(fit1)[1])`| 0                           |`r brformat(15*coef(fit1)[3])`| 0                            |0          |51,59|
| (3)=(1)+(2)   |`r brformat(coef(fit1)[1])`|`r brformat(5*coef(fit1)[2])`|`r brformat(15*coef(fit1)[3])`| 0                            |0          |56,60|
| (4)10% aclive |`r brformat(coef(fit1)[1])`| 0|0                         |`r brformat(.10^2*coef(fit)[4] + .10^3*coef(fit)[5])`        |0          |51,47|
| (5)10% declive|`r brformat(coef(fit1)[1])`| 0|0                         |`r brformat((-.10)^2*coef(fit)[4] + (-.10)^3*coef(fit)[5])`  |0          |53,61|
| (6)Pantanoso  |`r brformat(coef(fit1)[1])`| 0|0                         |0                             |`r brformat(coef(fit1)[6])`   |33,16      |     
...
|:--------------|--------:|------:|------:|----------:|-----------------:|
| Paradigma     | 1,00    | 1,00  | 1,00  | 1,00      | 1,0              |
| (1)+5m frente | 1,09    | 1,00  | 1,00  | 1,00      | 1,09             |
| (2)+15m fundo | 1,00    | 0,95  | 1,00  | 1,00      | 0,95             |
| (3)=(1)+(2)   | 1,09    | 0,95  | 1,00  | 1,00      | 1,04             |
| (4)10% aclive | 1,00    | 1,00  | 0,95  | 1,00      | 0,95             |
| (5)10% declive| 1,00    | 1,00  | 0,99  | 1,00      | 0,99             |
| (6)Pantanoso  | 1,00    | 1,00  | 1,00  | 0,61      | 0,61             |
...
jurere$AREA <- as.vector(scale(jurere$AREA, center = 600, scale = FALSE))
jurere$TESTADA <- as.vector(scale(jurere$TESTADA, center = 20, scale = FALSE))
...
fit <- lm(VU ~  I(AREA^2) + TESTADA + DIST_MAR + as.factor(PAVIMENTOS), 
          data = jurere, subset = -27)
summary(fit)
```

```{r}
newdata = data.frame(AREA = (600 - 600), TESTADA = (20 - 20), 
                     PROFUNDIDADE = (30-30), DIST_MAR = (1094 - mean_dist_mar)/sd_dist_mar, 
